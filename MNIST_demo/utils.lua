...
function utils.testModel(opt, model)
    local sample = opt.customSampleSize and torch.Tensor(torch.LongStorage(opt.customSampleSize)) or torch.Tensor(opt.batchSize, opt.imageChannel, opt.imageSize, opt.imageSize)
    local target = opt.customTargetSize and torch.Tensor(torch.LongStorage(opt.customTargetSize)) or torch.Tensor(opt.batchSize, opt.numClasses)
